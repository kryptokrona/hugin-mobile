  ...
  render() {
    ...
    const boardsRecommendations =
      <View style={{ paddingVertical: 12 }} >
        <TextField>{t('groupsRecommendations')}</TextField>
        <ScrollView
          ...
          {standardGroups?.length > 0 && standardGroups.map(function (item, i) {
            return <View key={`Rec-Group-${i}`}>
              <TouchableOpacity onPress={async () => { addGroup(item) }} style={[storyStyle, { backgroundColor: getBoardColors(item.key) }]}>
                <Text style={storyTextStyle}>{runes(item.name)[0].toUpperCase()}</Text>
              </TouchableOpacity>
              <Text style={{ width: 64, textAlign: 'center', fontFamily: 'Montserrat-Regular' }}>{item.name}</Text>
            </View>;
          })
          }
        </ScrollView>
    ...
    return (
      <ScreenLayout>
        <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginRight: 12 }} >
          <ScreenHeader>
            {t('groups')}
          </ScreenHeader>
        ...
        <View style={{ marginVertical: 12, justifyContent: 'space-between' }}>
    ...
    return (
      <View style={{
        ...
        <View style={{
          ...
          />
          <View style={{
            alignItems: 'center', marginTop: 8, borderRadius: 3, paddingTop: 0,
            borderColor: this.props.screenProps.theme.borderColour,
            borderWidth: 1,
          }}>
            <Button
